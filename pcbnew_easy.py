...
# a wrapper. However this was the quickest way to improve the usability for myself.
#
# All values are in mm, and coordinates and sizes can be given as iterables (tuples, lists, numpy arrays, ...)
#
...
class Board(object):
    ...
    def add_track_via(self, coord, layer_pair=(B_Cu, F_Cu), size=None, drill=None):
        ...
        via.SetLayerPair(layer_pair[0], layer_pair[1])
        via.SetDrill(drill)
        self.board.Add(via)
        return via
...
class Module(object):
    ...
    def add_pad(self, pos, size, name='', pad_type='standard', shape='circle', drill=1.0, layers=None):
